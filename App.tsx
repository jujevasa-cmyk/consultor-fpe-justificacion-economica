...
const App: React.FC = () => {
  ...
  const handleResetData = () => {
    if (window.confirm('¿Estás seguro de que deseas eliminar la base de datos actual y todas las justificaciones?')) {
      setData([]);
      setInputs({});
  ...
  const handleExportExcel = () => {
    const exportData = validData.map(action => {
        ...
        return {
            ...
            "Observaciones": input?.observaciones || '' // UPDATED Header Name
        };
    });
  ...
  const handleSaveCost = (input: JustificationInput) => {
    setInputs(prev => ({ ...prev, [input.actionId]: input }));
  };

  const toggleExpediente = (exp: string) => {
    setCollapsedExpedientes(prev => {
        const newSet = new Set(prev);
        if (newSet.has(exp)) newSet.delete(exp);
        else newSet.add(exp);
        return newSet;
    });
  };
  ...
  const availableExpedientes = useMemo(() => {
      ...
      if (activeCompany) filtered = filtered.filter(d => d.empresa === activeCompany);
  ...
  const dashboardStats = useMemo(() => {
    ...
    return { 
      totalConcedido, totalFinanciableReal, totalReconocido, accionesJustificadas, 
      totalActions: dashboardFilteredData.length, alumnoRetentionRate, empresaStats
    };
                    ...
                    return (
                        <tr key={action.id} className={`hover:bg-slate-50/80 transition-colors group ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                                ...
                                {result ? (
                                    <div className="flex flex-col items-end">
                                        ...
                                        {result.totalCostesIndirectosJustificados > result.limiteCostesIndirectos && (
                                            <span className="text-[9px] text-amber-500 line-through mt-0.5">
                            ...
                            <td className="px-4 py-4 text-center">
                                ...
                                {result && result.tipoDesviacion !== 'CUADRADO' && (
                                    <div className={`text-[9px] font-bold mt-1 px-2 py-0.5 rounded-md inline-block uppercase tracking-tight ${
                                        result.tipoDesviacion === 'DEFECTO' ? 'bg-blue-50 text-blue-600' : 'bg-rose-50 text-rose-600'
                                    }`}>
                                        {result.tipoDesviacion}: {formatCurrency(Math.abs(result.desviacion))}
                                    </div>
                                )}
                            </td>
                            <td className="px-4 py-4 text-right">
                                <div className="flex justify-end gap-2">
                                    <button onClick={() => setEditingAction(action)} className="p-1.5 text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-md border border-indigo-200 shadow-sm transition-colors"><Edit2 className="w-4 h-4" /></button>
                                    {result && input && (
                                        <button onClick={() => setAnalyzingAction({ action, input, result: result! })} className="p-1.5 text-purple-600 bg-purple-50 hover:bg-purple-100 rounded-md border border-purple-200 shadow-sm transition-colors"><BrainCircuit className="w-4 h-4" /></button>
                                    )}
                                </div>
                            </td>
                        </tr>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-800">
        ...
        {validData.length > 0 && (
            <div className="flex items-center gap-6 h-full">
                 <div className="flex items-center gap-2 overflow-x-auto no-scrollbar max-w-[calc(100vw-360px)] py-3">
                    <button onClick={() => setActiveTab('dashboard')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 whitespace-nowrap flex items-center gap-2 ${activeTab === 'dashboard' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-white text-slate-500 hover:text-indigo-600 hover:bg-indigo-50'}`}>
                        <LayoutDashboard className="w-4 h-4" /> Dashboard
                    </button>
                    {uniqueEmpresas.map(emp => (
                        <button key={emp} onClick={() => setActiveTab(emp)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 whitespace-nowrap flex items-center gap-2 ${activeTab === emp ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'bg-white text-slate-500 hover:text-indigo-600 hover:bg-indigo-50'}`}>
                            <Building2 className="w-4 h-4" /> {emp}
                        </button>
                    ))}
                 </div>
                 <div className="h-8 w-px bg-gray-200"></div>
                 <div className="flex gap-2">
                    <button onClick={handleExportExcel} className="p-2 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors border border-emerald-200 shadow-sm"><FileSpreadsheet className="w-5 h-5" /></button>
                    <button onClick={handleResetData} className="p-2 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors border border-transparent hover:border-rose-200"><Trash2 className="w-5 h-5" /></button>
                    <button onClick={() => setShowExitModal(true)} className="flex items-center gap-2 px-3 py-2 text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-lg transition-colors border border-rose-200 shadow-sm font-medium text-sm"><LogOut className="w-4 h-4" /> Salir</button>
                 </div>
            </div>
        )}
      </header>
          ...
          <div className="flex flex-col items-center justify-center py-20 animate-in fade-in slide-in-from-bottom-4">
             <div className="max-w-md w-full text-center">
                <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
                    <div className="bg-indigo-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6"><FileSpreadsheet className="w-10 h-10 text-indigo-600" /></div>
            ...
            {activeTab === 'dashboard' && (
                <div className="space-y-8 animate-in fade-in duration-300">
                     <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200 flex flex-wrap items-center gap-4">
                        <div className="flex items-center gap-2 text-slate-500 text-sm font-bold uppercase tracking-wider"><Filter className="w-4 h-4" /> <span>Filtros Globales:</span></div>
                        <div className="relative">
                            <select className="bg-gray-50 border border-gray-300 text-slate-700 text-sm rounded-md focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 pr-8 min-w-[200px]" value={filterEmpresa} onChange={(e) => { setFilterEmpresa(e.target.value); setFilterExpediente('ALL'); }}>
                                <option value="ALL">Todas las Empresas</option>
                                {uniqueEmpresas.map(emp => ( <option key={emp} value={emp}>{emp}</option> ))}
                            </select>
                        </div>
                        <div className="relative">
                             <select className="bg-gray-50 border border-gray-300 text-slate-700 text-sm rounded-md focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 pr-8 min-w-[200px]" value={filterExpediente} onChange={(e) => setFilterExpediente(e.target.value)}>
                                <option value="ALL">Todos los Expedientes</option>
                                {availableExpedientes.map(exp => ( <option key={exp} value={exp}>{exp}</option> ))}
                            </select>
                        </div>
                        <div className="relative">
                             <select className="bg-gray-50 border border-gray-300 text-slate-700 text-sm rounded-md focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 pr-8" value={filterSituacion} onChange={(e) => setFilterSituacion(e.target.value)}>
                        ...
                        <div className="relative flex-1 min-w-[200px]">
                            ...
                            <input type="text" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 p-2.5" placeholder="Buscar por Nombre o Código..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                            {searchTerm && ( <button onClick={() => setSearchTerm('')} className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-gray-600"><X className="w-4 h-4" /></button> )}
                        </div>
                     </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
                            <div className="px-6 py-5 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                                <h3 className="font-bold text-slate-800 flex items-center gap-2"><Building2 className="w-5 h-5 text-indigo-500" /> Rendimiento por Empresa</h3>
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {Object.entries(dashboardStats.empresaStats).map(([name, stats]: [string, CompanyStats]) => (
                                            <tr key={name} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-center text-sm text-slate-500"><span className="bg-slate-100 px-2 py-1 rounded-md font-medium">{stats.expedientes.size}</span></td>
                                                ...
                                                <td className="px-6 py-4 text-right text-sm text-slate-600"><span className={`px-2 py-1 rounded font-bold text-xs ${stats.financiableReal > 0 && stats.reconocido/stats.financiableReal < 0.7 ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600'}`}>{stats.financiableReal > 0 ? ((stats.reconocido / stats.financiableReal) * 100).toFixed(1) : 0}%</span></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex flex-col items-center justify-center">
                             <h3 className="text-sm font-bold text-slate-400 uppercase mb-6 self-start flex items-center gap-2"><PieChart className="w-4 h-4" /> Distribución Global</h3>
                             <div className="w-full h-[250px] relative">
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie data={chartData} cx="50%" cy="50%" innerRadius={70} outerRadius={90} dataKey="value" paddingAngle={4} cornerRadius={6}>
                                            {chartData.map((entry, index) => ( <Cell key={`cell-${index}`} fill={entry.color} strokeWidth={0} /> ))}
                                        </Pie>
                                    </PieChart>
                                </ResponsiveContainer>
                                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                    <span className="text-3xl font-bold text-slate-800">{dashboardStats.totalFinanciableReal > 0 ? ((dashboardStats.totalReconocido / dashboardStats.totalFinanciableReal) * 100).toFixed(0) : 0}%</span>
                ...
                <div className="animate-in fade-in duration-300 space-y-8">
                    {(() => {
                        ...
                        const filteredActions = companyData.filter(action => {
                            ...
                            const matchSit = filterSituacion === 'ALL' || status === filterSituacion;
                            const matchText = searchTerm === '' || action.denominacion.toLowerCase().includes(searchTerm.toLowerCase()) || action.codigoAccion.toLowerCase().includes(searchTerm.toLowerCase());
                        ...
                        const companyTotals = companyData.reduce((acc, action) => {
                             ...
                             if (input) finalStudents = input.situacion === 'NO_EJECUTABLE' ? 0 : input.alumnosFinalizados;
                             acc.alumnosFin += finalStudents;
                             return acc;
                        }, { concedido: 0, justificado: 0, pagado: 0, pendiente: 0, alumnosConc: 0, alumnosFin: 0 });
                        const retention = companyTotals.alumnosConc > 0 ? (companyTotals.alumnosFin / companyTotals.alumnosConc) * 100 : 0;
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-1 mb-6">
                                <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white rounded-t-xl flex justify-between items-center">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-indigo-100 rounded-xl flex items-center justify-center text-indigo-600 shadow-sm"><Building2 className="w-7 h-7" /></div>
                                        <div><h2 className="text-2xl font-bold text-slate-800">{activeTab}</h2><p className="text-sm text-slate-500">Resumen Ejecutivo y Estado de Situación</p></div>
                                    </div>
                                    <div className="flex gap-4">
                                        <div className="text-right px-4 border-r border-gray-200"><p className="text-xs text-slate-400 font-bold uppercase">Expedientes Activos</p><p className="text-xl font-bold text-slate-800">{allExpedients.length}</p></div>
                                        <div className="text-right"><p className="text-xs text-slate-400 font-bold uppercase">Acciones Totales</p><p className="text-xl font-bold text-slate-800">{companyData.length}</p></div>
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                                    <div className="p-6 bg-gradient-to-b from-white to-gray-50/30">
                                        <h4 className="text-xs font-bold text-indigo-900 uppercase flex items-center gap-2 mb-4 bg-indigo-50 w-fit px-2 py-1 rounded"><BarChart3 className="w-3.5 h-3.5" /> Ejecución Eco</h4>
                                        <div className="space-y-4">
                                            <div className="flex justify-between items-end"><span className="text-sm text-slate-500">Concedido</span><span className="font-bold text-slate-800">{formatCurrency(companyTotals.concedido)}</span></div>
                                            <div className="flex justify-between items-end"><span className="text-sm text-slate-500">Justificado</span><span className="font-bold text-indigo-700 text-lg">{formatCurrency(companyTotals.justificado)}</span></div>
                                        </div>
                                    </div>
                                    <div className="p-6 bg-gradient-to-b from-white to-violet-50/10">
                                        <h4 className="text-xs font-bold text-violet-900 uppercase flex items-center gap-2 mb-4 bg-violet-50 w-fit px-2 py-1 rounded"><Landmark className="w-3.5 h-3.5" /> Liquidación Admin</h4>
                                        <div className="space-y-4">
                                            <div className="flex justify-between items-center"><p className="text-xs font-semibold text-slate-500">Anticipo (70%)</p><p className="text-sm font-bold text-blue-600">{formatCurrency(totalAnticipo)}</p></div>
                                            <div className={`p-3 rounded-lg border ${totalLiquidacion >= 0 ? 'bg-indigo-50 border-indigo-100' : 'bg-orange-50 border-orange-100'}`}>
                                                 <div className="flex justify-between items-center mb-1"><span className={`text-[10px] font-bold uppercase ${totalLiquidacion >= 0 ? 'text-indigo-800' : 'text-orange-800'}`}>{totalLiquidacion >= 0 ? 'A Cobrar' : 'A Devolver'}</span></div>
                                                 <p className={`text-xl font-bold tracking-tight ${totalLiquidacion >= 0 ? 'text-indigo-700' : 'text-orange-700'}`}>{formatCurrency(Math.abs(totalLiquidacion))}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="p-6 bg-gradient-to-b from-white to-emerald-50/10">
                                        <h4 className="text-xs font-bold text-emerald-900 uppercase flex items-center gap-2 mb-4 bg-emerald-50 w-fit px-2 py-1 rounded"><Wallet className="w-3.5 h-3.5" /> Tesorería Real</h4>
                                        <div className="space-y-3">
                                            <div className="pt-3 border-t border-gray-100">
                                                <div className="flex justify-between items-center mb-1"><p className="text-[10px] text-slate-400 uppercase font-bold">Caja (Liquidez)</p></div>
                                                <div className="flex items-center gap-2"><PiggyBank className={`w-6 h-6 ${saldoCaja >= 0 ? 'text-emerald-500' : 'text-rose-500'}`} /><p className={`text-2xl font-bold ${saldoCaja >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{formatCurrency(saldoCaja)}</p></div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="p-6 bg-gradient-to-b from-white to-blue-50/10">
                                        <h4 className="text-xs font-bold text-blue-900 uppercase flex items-center gap-2 mb-4 bg-blue-50 w-fit px-2 py-1 rounded"><Users className="w-3.5 h-3.5" /> Impacto Alumnos</h4>
                                        <div className="space-y-4"><div className={`p-2 rounded flex items-center justify-between text-xs font-bold ${retention < 85 ? 'bg-amber-50 text-amber-700' : 'bg-green-50 text-green-700'}`}><span>Tasa Retención</span><span>{retention.toFixed(1)}%</span></div></div>
                                    </div>
                                </div>
                            </div>
                            <div className="bg-gray-100 rounded-lg p-3 mb-6 flex flex-wrap items-center gap-4 shadow-inner border border-gray-200">
                                <div className="flex items-center gap-2 text-slate-500 text-sm font-semibold"><Filter className="w-4 h-4" /><span>Filtrar Vista:</span></div>
                                <select className="bg-white border border-gray-300 text-slate-700 text-sm rounded-md focus:ring-indigo-500 focus:border-indigo-500 block p-2" value={activeTab} onChange={(e) => setActiveTab(e.target.value)}>
                                    {uniqueEmpresas.map(emp => ( <option key={emp} value={emp}>{emp}</option> ))}
                                </select>
                                <select className="bg-white border border-gray-300 text-slate-700 text-sm rounded-md focus:ring-indigo-500 focus:border-indigo-500 block p-2 min-w-[150px]" value={filterExpediente} onChange={(e) => setFilterExpediente(e.target.value)}>
                                    <option value="ALL">Todos los Expedientes</option>
                                    {allExpedients.map(exp => ( <option key={exp} value={exp}>{exp}</option> ))}
                                </select>
                                <div className="relative flex-1 min-w-[200px]">
                                    <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none"><Search className="w-4 h-4 text-gray-400" /></div>
                                    <input type="text" className="bg-white border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 p-2" placeholder="Buscar por Nombre o Código..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                                </div>
                            </div>
                            ...
                            {visibleExpedients.map((exp: string) => {
                                    ...
                                    return (
                                        <div key={exp} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden transition-all duration-300">
                                            <div className="bg-white p-1 cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => toggleExpediente(exp)}>
                                                <div className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                                    <div className="flex items-center gap-4"><button className={`p-2 rounded-full border transition-all ${isCollapsed ? 'bg-gray-100 text-gray-400 border-gray-200' : 'bg-indigo-50 text-indigo-600 border-indigo-200'}`}>{isCollapsed ? <ChevronDown className="w-5 h-5" /> : <ChevronUp className="w-5 h-5" />}</button><div><div className="flex items-center gap-2"><FileText className="w-4 h-4 text-slate-400" /><h2 className="text-lg font-bold text-slate-800">{exp}</h2></div></div></div>
                                                    <div className="flex items-center gap-6"><div className="text-right"><p className="text-[10px] font-bold text-indigo-400 uppercase">Justificado</p><p className="text-xl font-bold text-indigo-700">{formatCurrency(totalReconocido)}</p></div></div>
                                                </div>
                                            </div>
                                            <div className={`transition-all duration-300 ease-in-out ${isCollapsed ? 'max-h-0 opacity-0' : 'max-h-[2000px] opacity-100'}`}>{!isCollapsed && renderTable(expData)}</div>
                                        </div>
                                    );
                                })
                            }
      ...
      {editingAction && ( <CostEditor action={editingAction} existingInput={inputs[editingAction.id]} onSave={handleSaveCost} onClose={() => setEditingAction(null)} /> )}
      {analyzingAction && ( <AnalysisModal action={analyzingAction.action} input={analyzingAction.input} result={analyzingAction.result} onClose={() => setAnalyzingAction(null)} /> )}
      {showExitModal && (
        <div className="fixed inset-0 bg-black/60 z-[70] flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="p-6 text-center">
                    <div className="bg-amber-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"><HardDriveDownload className="w-8 h-8 text-amber-600" /></div>
                    <h3 className="text-xl font-bold text-slate-800 mb-2">¿Quieres guardar antes de salir?</h3>
                    <div className="flex flex-col gap-3"><button onClick={handleExitAndDownload} className="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold shadow-md flex items-center justify-center gap-2 transition-colors"><Download className="w-5 h-5" /> Sí, Descargar Copia Excel</button><button onClick={() => { setShowExitModal(false); }} className="w-full py-3 px-4 bg-white border border-gray-300 hover:bg-gray-50 text-slate-700 rounded-lg font-medium transition-colors">No, solo cerrar</button></div>
                    <button onClick={() => setShowExitModal(false)} className="mt-4 text-sm text-slate-400 hover:text-slate-600 underline">Cancelar y volver</button>
                </div>
            </div>
        </div>
      )}
